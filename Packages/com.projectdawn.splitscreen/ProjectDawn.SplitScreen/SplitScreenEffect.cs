...
namespace ProjectDawn.SplitScreen
{
    ...
    public class SplitScreenEffect : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void RemoveScreen(Transform target)
        {
            CheckIsCreated();
            // Find the screen that matches the target transform
            var screenToRemove = Screens.Find(screen => screen.Target == target);
            if (screenToRemove != null)
            {
                Destroy(screenToRemove.Camera.gameObject);
